    ...
    public int scrollHorizontallyBy(int dx, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (scaleOnScroll) {
            ...
            for (int i = 0; i < getChildCount(); i++) {
                ...
                int childWidth = child.getRight() - child.getLeft();
                float childWidthHalf = childWidth / 2.f;
                float childCenter = child.getLeft() + childWidthHalf;

                int parentWidth = isOffsetStart ? childWidth : getWidth();
                ...
                float delta = Math.min(d1, Math.abs(parentWidthHalf - childCenter));

                float position = s0 + (s1 - s0) * (delta - d0) / (d1 - d0);
